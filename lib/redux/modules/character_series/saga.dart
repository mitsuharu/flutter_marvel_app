...
Iterable characterSeriesSaga() sync* {
  ...
  yield TakeEvery(_requestCharacterSiriesSaga, pattern: ClearAndRequestCharacterSeries);
...
Iterable _requestCharacterSiriesSaga({dynamic action}) sync* {
  ...
  yield Try(() sync* {
    ...
    if ( action is RequestCharacterSeries || action is ClearAndRequestCharacterSeries){
      ...
      yield Call((){
        appDatabase.insertSeriesFromApi(action.characterId, response.value!.data.results);
      }); 
    }else{
